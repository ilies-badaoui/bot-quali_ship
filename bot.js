...
const loginChronopostTickets = require("./src/transporters/chronopost/chronopostTickets");
...
async function startAutomation() {
  ...
  try {
    logger.info("Lancement de la vérification des tickets de Chronopost...");
    await loginChronopostTickets();
    logger.info("Vérification des tickets de Chronopost terminé avec succès.");
  } catch (error) {
    logger.warn(
      "Erreur lors de la vérification tickets de Chronopost :",
      error.message
    );
  }
  ...
  try {
    ...
    logger.warn(
      "Erreur ou absence de nouvelles réclamations lors de l'export de Colissimo :",
      error.message
    );
  }

  logger.info(
    "Toutes les opérations ont été effectuées (qu'elles aient réussi ou échoué)."
  );
}
